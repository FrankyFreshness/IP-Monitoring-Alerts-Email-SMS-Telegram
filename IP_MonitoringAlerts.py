    ...
    for ip in park:
     ...
     #Sends an Alert if the result of the ping is "Request Timed Out"
        if "Request timed out." in response:
            ...
            TelegramBotAlert.bot.sendMessage(TelegramBotAlert.receiver_id, ip + ' is down (Request Timed Out).\n\nPlease reply with "Ack" to pause alerts for 60 minutes') #Sends a Telegram message as a bot
        ...
        if "Destination host unreachable." in response:
            ...
            TelegramBotAlert.bot.sendMessage(TelegramBotAlert.receiver_id, ip + ' is down (Destination host unreachable).\n\nPlease reply with "Ack" to pause alerts for 60 minutes')
            ...
            if TelegramBotAlert.TeleAckChecker != 1:
                LoggingAlerts.logalert_no_ack()  # Logs that alert has NOT been acknowledged
            time.sleep(60)  # If error is sent, wait 60 seconds before sending another message (Or however long you would like)
